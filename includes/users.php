                    ...
                    <table width="100%">
                        <tbody>
                            <tr>
                                ...
                                <td width="4"></td>
                                <?php
                                    $messageStatemant = $conn->prepare('SELECT * FROM messages WHERE sender_id = :sender_id OR receiver_id = :receiver_id ORDER BY id DESC LIMIT 1');
                                    $messageStatemant->execute([
                                        'sender_id'=>  $_COOKIE['id'],
                                        'receiver_id'=> $_COOKIE['id'],
                                    ]);
                                    $message = $messageStatemant->fetchAll();
                                ?>
                                <td>
                                    <div class="user-fullname"><?=$user['email'] ?></div>
                                    <?=count($message) == 0 ? "<i>Pas de message ... </i>" : (strlen($message[0]['text']) > 20 ? substr($message[0]['text'], 0, 20) . "..." : $message[0]["text"]) ?>
                                </td>
                                <td width="20">
                                    <small>12:45</small>
                                </td>
